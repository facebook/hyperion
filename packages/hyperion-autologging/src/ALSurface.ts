...
export type ALSurfaceEventData = GenericSurfaceEventData<WebSurfaceEventExtension>;

export type ALSurfaceCapability = SurfaceCapability;

export type ALSurfaceProps = BaseSurfaceProps & Readonly<{
  nodeRef?: React.RefObject<HTMLElement | null | undefined>;
}>;

export type ALSurfaceRenderer = GenericSurfaceRenderer;
export type ALSurfaceHOC = GenericSurfaceHOC<ALSurfaceProps>;
export type ALSurfaceRenderers = GenericSurfaceRenderers<SurfaceComponent, ALSurfaceProps>;
export type ALChannelSurfaceEvent = GenericChannelSurfaceEvent<ALSurfaceEventData>
...
const { registry: alSurfaceDataRegistry, dataRegistry: alDataRegistry } = createSurfaceDataRegistry<ALSurfaceData>();
...
export function init(options: InitOptions): ALSurfaceRenderers {
  ...
  const SurfaceWithEvent = createSurfaceWithEventComponent<ALSurfaceData, ALSurfaceEventData>(
  ...
  const Surface: SurfaceComponent = props => {
    ...
    if (proxiedContext?.container instanceof Element) {
      ...
      if (container.childElementCount === 0 || container.getAttribute(attributeName) === attributeValue) {
...
export function getALSurfaceData(nonInteractiveSurfacePath: string): ALSurfaceData | undefined {
  return alSurfaceDataRegistry.get(nonInteractiveSurfacePath);
}

export function clearALSurfaceRegistry(): void {
  alSurfaceDataRegistry.clear();
}

export function getAllALSurfaces(): Map<string, ALSurfaceData> {
  return new Map(alSurfaceDataRegistry);
}

/**
 * Web-specific surface wrapper creation
 */
