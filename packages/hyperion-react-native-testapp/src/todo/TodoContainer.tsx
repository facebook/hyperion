...
const TodoContainer = () => {
  ...
  const [todoListMetas, setTodoListMetas] = useState<TodoListMeta[]>(
    initialTodoListMetas
  );
  const [activeTabId, setActiveTabId] = useState<string>(
    initialTodoListMetas[0]?.id || ''
  );
  const [activeTabName, setActiveTabName] = useState<string>(
    initialTodoListMetas[0]?.name || ''
  );
  const [newListName, setNewListName] = useState('');

  const listManagersRef = useRef<Map<string, IndividualTodoListManager>>(
    new Map()
  );
